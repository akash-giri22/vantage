...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    const saved =
      typeof window !== 'undefined'
        ? localStorage.getItem('vantage-theme')
        : null;
    ...
    api.getApplications()
      .then((rows) => {
        ...
        const count = rows.filter((row) =>
          String(row.created_at || '').slice(0, 10) === today
        ).length;

        setAppliesToday(count);
      })
      .catch(() => {});
  }, [router.pathname]);
  ...
  const progress = Math.min(100, (appliesToday / DAILY_APPLY_CAP) * 100);
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      <div className="md:hidden sticky top-0 z-20 flex items-center justify-between px-4 py-3 border-b border-line bg-bg/90 backdrop-blur">
        ...
        <div className="font-mono text-xs text-inksoft bg-surface2 px-2.5 py-1 rounded-full">
          {appliesToday} / {DAILY_APPLY_CAP} applies
        </div>
      </div>
          ...
          {profileOpen && (
            <div className="absolute bottom-full right-0 md:left-0 mb-2 w-40 bg-surface2 border border-line rounded-lg p-1.5 shadow-xl">
              <Link href="/settings" legacyBehavior>
                <a className="block px-2.5 py-2 rounded-md text-sm hover:bg-surface">
                  Settings
                </a>
              </Link>
              ...
              <button
                onClick={toggleTheme}
                className="w-full text-left px-2.5 py-2 rounded-md text-sm hover:bg-surface"
              >
      ...
      <main className="order-1 md:order-2 flex-1 px-5 md:px-11 py-6 md:py-9 pb-24 md:pb-9 max-w-5xl">
        {children}
      </main>
    </div>
  );
